...
export function initialize(): void {
	...
	game?.keybindings?.register(MODULE_NAME, "flipTile", {
		...
		editable: [{ key: "KeyG" }],
		onDown: (event) =>
			void mirrorSelectedTiles(
				event.isShift ? TileMirror.VERTICAL : TileMirror.HORIZONTAL,
			),
	...
	async function mirrorSelectedTiles(
		tileMirrorDirection: TileMirror,
	): Promise<void> {
	...
	function updateTile(tile: foundry.canvas.placeables.Tile) {
		...
		const flipVertical = tile.document.getFlag(
			MODULE_NAME,
			TileMirror.VERTICAL,
		);
